    ...
    const cartHTML = cartItems
      .map((item) => {
        return `<div class="cart__item">
                                <img src="${item.img}" alt="${item.title}" class="cart__item__img">
                                <div class="cart__item__detail">
                                    <h3 class='detail__heading'>${item.title}</h3>
                                    <p>${item.description}</p>
                                    <p class="price">$${item.price}</p>
                                    <div class="cart__item__count">
                                        <button class='count__btn' onClick="decreaseCount(${item.id})">-</button>
                                        <span class='item__count'>${item.count}</span>
                                        <button class='count__btn' onClick="increaseCount(${item.id})">+</button>
                                    </div>
